...
public class AggregatedMetricsRepository {
        ...
        return getConnection()
                .map(conn -> {
                    try {
                        ...
                        if (sort != null) {
                            query.append(sort.toOrderString(groupBy));
                        }
                        if (limit != null) {
                            query.append(" LIMIT ?");
                            params.add(limit);
    ...
    public enum SortableColumn {
        ...
        public static SortableColumn fromString(String value) {
            for (var column : SortableColumn.values()) {
                if (column.inputId.equals(value)) {
                    return column;
                }
            }
            throw new BadRequestException(String.format("Invalid sort column '%s'. Expected one of ", value)
                    + Stream.of(SortableColumn.values())
                    .map(v -> v.inputId)
                    .collect(Collectors.joining(", ")));
        }
    }

    public record SortSpec(LinkedHashMap<SortableColumn, SortOrder> columns) {
        public static SortSpec fromString(String value) {
            var parts = value.split(",");
            var columns = new LinkedHashMap<SortableColumn, SortOrder>();
            try {
                for (var part : parts) {
                    var split = part.split(":");
                    var column = SortableColumn.fromString(split[0]);
                    var order = SortOrder.fromString(split[1]);
                    columns.put(column, order);
                }
            } catch (BadRequestException e) {
                throw e;
            } catch (Exception e) {
                throw new BadRequestException("Invalid sort format. Expected comma-separated list of column:order pairs, e.g. startTime:asc,endTime:desc", e);
            }
            return new SortSpec(columns);
        }

        public String toOrderString(@Nullable GroupBySpec groupBy) {
            var groupByStartTime = groupBy != null && groupBy.groupByStartTime;
            var groupByEndTime = groupBy != null && groupBy.groupByEndTime;
            if (columns.isEmpty()) {
                return "";
            }
            // make sure we do not sort by columns that are not in the group by clause (note: 'value' is generated by the aggregation function)
            return columns.entrySet().stream()
                    .filter(e -> switch (e.getKey()) {
                        case START_TIME -> groupByStartTime;
                        case END_TIME -> groupByEndTime;
                        default -> true;
                    })
                    .map(e -> String.format("%s %s", e.getKey().columnName, e.getValue()))
    ...
    public enum SortOrder {
        ...
        public static SortOrder fromString(String value) {
            try {
                ...
                throw new BadRequestException(String.format("Invalid sort order '%s'. Expected 'asc' or 'desc'", value));
            }
        }
    }
